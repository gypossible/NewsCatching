...
现在也支持上传 Excel / CSV 附件，先配置读取规则，再按指定列批量监测并把结果写回附件。
...
6. 点击“批量监测并写回”，任务完成后再点“导出写回附件”即可下载带结果列的新文件。
...
```text
舆情任务状态 | 风险等级 | 风险评分 | 情绪判断 | 高频主题 | 监测摘要 | 建议动作 | 写入时间
```
...
- 如果批量结果很多，页面不会展开全部报告卡片，但会完整写回到导出文件里。
